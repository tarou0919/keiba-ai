...
def get_model():
    ...
    X = pd.DataFrame({
        ...
        "venue_num":              np.random.randint(0, 10, n),
        "age":                    np.random.randint(3, 8, n),
        "sex_num":                np.random.choice([0,1,2], n, p=[0.6,0.35,0.05]),
        "weight_carried":         np.random.choice([54,55,56,57,58], n),
        "horse_weight_kg":        np.random.normal(490, 20, n),
        "weight_change":          np.random.choice([-6,-4,-2,0,2,4,6], n),
        "odds_num":               np.abs(np.random.exponential(8, n)) + 1.1,
        "favorite_num":           np.random.randint(1, 18, n),
        "log_odds":               np.log(np.abs(np.random.exponential(8, n)) + 1.1),
        "jockey_win_rate":        np.random.uniform(0.05, 0.25, n),
        "jockey_place_rate":      np.random.uniform(0.15, 0.55, n),
        "trainer_win_rate":       np.random.uniform(0.03, 0.22, n),
        "horse_avg_rank_last3":   np.random.uniform(1, 16, n),
        "horse_avg_rank_last5":   np.random.uniform(1, 16, n),
        "horse_best_rank":        np.random.randint(1, 6, n),
        "horse_win_count":        np.random.randint(0, 10, n),
        "horse_race_count":       np.random.randint(1, 30, n),
        "horse_win_rate":         np.random.uniform(0, 0.4, n),
        "same_course_win_rate":   np.random.uniform(0, 0.3, n),
        "same_dist_win_rate":     np.random.uniform(0, 0.3, n),
        "days_since_last_race":   np.random.choice([28,35,42,56,70,84], n),
        "race_interval_score":    np.random.uniform(0.3, 1.0, n),
        "speed_index":            np.random.normal(100, 5, n),
        "speed_index_avg3":       np.random.normal(100, 5, n),
    })
    jw = X["jockey_win_rate"]
    hw = X["horse_weight_kg"]
    strength = (
        -X["odds_num"] * 0.06
        + jw * 4
